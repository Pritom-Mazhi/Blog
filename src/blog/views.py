...
#homepageview i created at the last for my remembering
class HomeView(TemplateView):
    template_name = 'home.html'

    def get_context_data(self, **kwargs):
        ctxt = super(HomeView, self).get_context_data(**kwargs)
        ...
        if self.request.user.is_authenticated():
            if Blog.objects.filter(owner=self.request.user).exists():
                ...
                #ctxt['blog'] = Blog.objects.get(owner=self.request.user).exists()
                ctxt['blog'] = blog
                ctxt['blog_posts'] = BlogPost.objects.filter(blog=blog)
                ctxt['shared_posts'] = blog.shared_posts.all()
        return ctxt
